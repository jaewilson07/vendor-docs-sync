...
<Accordion title="Can I run the Workbench service under a Windows service account while individual users run the Workbench client with their own logins?">

Yes, and this is a recommended setup. Have an IT administrator (or whoever manages the server) install Workbench on the machine and authenticate it with a Domo service account so that it isn't tied to an individual user.

Then add each user who needs to use Workbench to the **Workbench Users** local security group. For more information, see [Active Directory security groups](https://learn.microsoft.com/en-us/windows-server/identity/ad-ds/manage/understand-security-groups) on Microsoft Learn. After a user is added to the group, they can access Workbench to create, run, and monitor jobs.

<Note>
**Note:** This works well as long as multiple users aren't editing the same things at the same time. When that happens, Workbench displays **Live updates are not available** in red text at the top of the window.
</Note>
</Accordion>
