...
In step 12, instead of outputting the entire table with `SELECT * FROM final`, you can update only a segment of the data. Use a `UNION ALL` query that filters each `SELECT` by a date range, so that only the rows in the segment you choose are refreshed while the rest of the data is preserved. For example, the following query keeps records older than two months from one table and the most recent two months of records from another:
