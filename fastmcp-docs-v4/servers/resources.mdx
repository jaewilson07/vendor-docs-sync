...
A single value still arrives as a one-element list, so the parameter's type is stable. Declaring a parameter that can *only* be a list without the `*` raises an error when the template is created. A parameter that also accepts a scalar (`str | list[str] | None`) is left alone — it still works as a plain `{?param}`.
...
4. **Query parameters that can only be a list** must use the explode modifier (`{?param*}`)
