...
**Pydantic AI users need `pydantic-ai-slim` 2.45 or newer.** Releases 2.32 through 2.44 import `httpx` in their MCP module without depending on it. FastMCP 3 installed `httpx`, which hid this; with FastMCP 4, `import pydantic_ai.mcp` fails. See [Pydantic AI](/integrations/pydantic-ai).
...
1. **Bump your environment.** Raise any pin below `pydantic>=2.12` or `pydantic-ai-slim>=2.45`; upgrade FastAPI if your resolver complains about Starlette `<1.0.1`.
